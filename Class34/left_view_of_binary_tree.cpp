...
void leftViewOfBT_DFS(TreeNode*root, int curLevel, map<int, int> &levelVsNodeValMap){
	if(!root)
		return;

	if(levelVsNodeValMap.find(curLevel) == levelVsNodeValMap.end())
		levelVsNodeValMap[curLevel] = root->val;

	leftViewOfBT_DFS(root->left, curLevel+1, levelVsNodeValMap);
	leftViewOfBT_DFS(root->right, curLevel+1, levelVsNodeValMap);
}

void leftViewOfBT(TreeNode*root){
	map<int, int> levelVsNodeValMap;
	leftViewOfBT_DFS(root, 0, levelVsNodeValMap);
